...
class NatureQN(Linear):
    ...
    def initialize_models(self):
        ...
        '''def dqn():
            ...
            x = tf.keras.layers.Conv2D(32, (8, 8), strides = 4, padding="same", kernel_initializer = initializer)(inputs)
            x = tf.keras.layers.Activation('relu')(x)
            #second encoding layer
            x = tf.keras.layers.Conv2D(32, (4, 4), strides = 2, padding="same", kernel_initializer = initializer)(x)
            x = tf.keras.layers.Activation('relu')(x)
            #third encoding layer
            x = tf.keras.layers.Conv2D(64, (3, 3), strides = 1, padding="same", kernel_initializer = initializer)(x)
            x = tf.keras.layers.Activation('relu')(x)
            #flatten
        ...
        self.q_network = nn.Sequential(
            nn.Conv2d(in_channels=4, out_channels=32, kernel_size=8, stride=4, padding=4),
            nn.ReLU(),
            ...
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(in_features=7744,out_features=512),
            nn.ReLU(),
