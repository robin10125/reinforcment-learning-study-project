...
class Linear(DQN):
    ...
    def get_q_values(self, state, network='q_network'):
        ...
        Hint:
            ...
            ->my note: Dense layer doesn't accept multiple dimensions, must manually convert to 1 dimension beforehand
        ...
        out = model(torch.flatten(state, start_dim=1))
    ...
    def calc_loss(self, q_values : Tensor, target_q_values : Tensor,
        ...
        ##############################################################
        ##################### YOUR CODE HERE - 3-5 lines #############
        tq_max_val = torch.max(target_q_values, 1).values
        discount_max_qt = rewards + (gamma * tq_max_val * (-1*done_mask + 1) )
        #value is equal to: rewards + max of target_q_vlaues(in each experience of batch), scaled by gamma, 
        #and then mulitplied by either 0 or 1 depending on whether terminal state is reached

        qc = torch.sum(q_values * F.one_hot(actions.long(), num_actions), 1)
        #reduce new tensor (equal to: one-hot embedding of actions * q vals for each action) to one dimension
        #Will sum a bunch of zeros and 1 non zero value for each experience in batch, leaving 1 dimension tensor of chosen action values
        
        loss = F.mse_loss(discount_max_qt, qc)
        return loss
        ##############################################################
        ######################## END YOUR CODE #######################
